...
public class SwiftIcloudStoragePlugin: NSObject, FlutterPlugin {
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "iCloudPermission":
            iCloudPermission(call, result)
        default:
            result(FlutterMethodNotImplemented)
        }
    }
    
    private func iCloudPermission(_ call: FlutterMethodCall, _ result: @escaping FlutterResult){
        guard FileManager.default.url(forUbiquityContainerIdentifier: containerId) != nil
        else {
            result(false)
            return
        }
